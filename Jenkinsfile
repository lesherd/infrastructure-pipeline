...
node('linux') {
	...
	sh "aws ec2 describe-instances --region us-east-1 | jq '[.Reservations | .[] | .Instances | .[] | {InstanceId: .InstanceId, State: .State, SubnetId: .SubnetId, VpcId: .VpcId, Name: (.Tags[]|select(.Key==\"Name\")|.Value)}]'"
    }	    
